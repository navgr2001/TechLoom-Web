		...
		protected function wkit_generate_post_data (){			
			...
			$post_types = [
				[
					"id" => 19509,
					"type" => "Company Story",
					"description" => "A blog title about the origin of {{site_name}}, why it started, its mission, and what makes it stand out today."
				],
				[
					"id" => 19510,
					"type" => "Industry Problems & Solutions",
					"description" => "A title discussing 3 common challenges in the {{site_type}} industry and how {{site_name}} solves them."
				],
				[
					"id" => 19511,
					"type" => "Process or Experience",
					"description" => "A title that explains how {{site_name}} delivers its services/products, and guides customers through buying, booking, or engaging."
				],
				[
					"id" => 19512,
					"type" => "Success Stories & Achievements",
					"description" => "A title highlighting milestones, customer success stories, or awards of {{site_name}}."
				],
				[
					"id" => 19513,
					"type" => "Choosing the Right Solution",
					"description" => "A title that helps readers choose the best {{site_type}} solution and subtly positions {{site_name}} as the ideal choice."
				],
				[
					"id" => 19514,
					"type" => "Culture & Team",
					"description" => "A title about the values, team spirit, and culture of {{site_name}} that drive its success."
				]
			];

			if (empty($site_type) || empty($site_title)) {
				wp_send_json([
					'success'     => false,
					'message'     => 'Invalid input: site_type and site_title are required.',
					'description' => 'Please make sure all fields are filled correctly before making the request.',
				]);
				wp_die();
			}

			$site_json = json_encode($post_types, JSON_PRETTY_PRINT);

			$gpt_prompt = <<<EOT
				You are a professional blog strategist and SEO expert.

				I will give you the following inputs:  
				- Site Name: {$site_name}  
				- Site Type: {$site_type}  
				- Site Description: {$site_description} (optional)  

				Based on these inputs and the blog post types below, do the following:  

				1. For each post type, generate a **unique and SEO-friendly blog post title**. Keep it concise (10–12 words max) and engaging.  
				2. Create a **global list of 6–8 blog categories** relevant to all 6 titles.  
				3. Create a **global list of 18–22 tags** that are relevant to the titles and categories.  
				4. For each blog post:  
				- Assign **1–2 categories** from the global list (max 2 per post and max to max only 3 post can have 2 category other allow only 1).  
				- Assign **4–5 tags** from the global tag list, highly relevant to the post’s topic.  

				### BLOG POST TYPES:
				{$site_json}

				### OUTPUT FORMAT:
				Return JSON only (no extra text) and do not wrap it in ```json ``` format. Structure it like this:

				{
				"posts": [
					{
					"id" : "already given in post type array ( do not change id )",
					"title": "Generated blog post title here",
				...
				"tags": ["Tag 1", "Tag 2", "Tag 3", ..., "Tag 20"]
				}
			EOT;

			$api_key = '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';

			$result = wp_remote_post('https://api.openai.com/v1/chat/completions', [
				'timeout' => 120,
				'headers' => [
